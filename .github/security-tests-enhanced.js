...
class SecurityTester {
  ...
  async testSSLConfiguration() {
    ...
    try {
      ...
      this.addResult(
        'warning',
        `Could not test SSL configuration: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      if (cookies) {
        ...
        if (hasSecure || this.baseURL.startsWith('http://localhost')) {
          this.addResult(
            'pass',
            'Session cookie has Secure flag (or is localhost)'
          );
        } else {
  ...
  async testAPIVersioning() {
    ...
    if (!hasVersioning) {
      this.addResult(
        'warning',
        'API versioning not detected - consider implementing for backward compatibility'
      );
    }
  }
    ...
    for (const test of testCases) {
      try {
        ...
        if (response.statusCode === test.expected) {
          ...
          this.addResult(
            'warning',
            `${test.name} returns ${response.statusCode} instead of ${test.expected}`
          );
        }
      } catch (error) {
  ...
  async testCORS() {
    ...
    const origins = ['https://evil.com', 'http://localhost:3000', 'null'];
    ...
    for (const origin of origins) {
      try {
        ...
        if (allowedOrigin === '*') {
          this.addResult(
            'fail',
            `CORS allows all origins (*) - this is insecure for authenticated endpoints`
          );
          ...
          this.addResult(
            'fail',
            `CORS allows 'null' origin - this can be exploited`
          );
        ...
        this.addResult(
          'warning',
          `Could not test CORS for origin ${origin}: ${error.message}`
        );
      }
    }
  }
