...
class TestMessageStore:
    ...
    @pytest.mark.asyncio
    async def test_read_nonexistent_channel(self) -> None:
        store = MessageStore()
    ...
    @pytest.mark.asyncio
    async def test_since_timestamp_filtering(self) -> None:
        store = MessageStore()
    ...
    @pytest.mark.asyncio
    async def test_since_invalid_timestamp(self) -> None:
        store = MessageStore()
    ...
    async def test_limit_capped_at_max(self) -> None:
        ...
        messages, has_more = await store.get("test", limit=99999)
    ...
    @pytest.mark.asyncio
    async def test_clear_channel(self) -> None:
        store = MessageStore()
    ...
    @pytest.mark.asyncio
    async def test_clear_nonexistent_channel(self) -> None:
        store = MessageStore()
        assert await store.clear("nope") is False

    @pytest.mark.asyncio
    async def test_clear_channel_persists_in_list(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_removes_from_store(self) -> None:
        store = MessageStore()
        await store.add("test", "msg1")
        await store.add("test", "msg2")
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_nonexistent(self) -> None:
        store = MessageStore()
        assert await store.delete("nope") is False

    @pytest.mark.asyncio
    async def test_delete_channel_removes_from_list(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_frees_slot_for_new_channel(self) -> None:
        ...
        await store.add("ch3", "msg")  # should not raise

        channels = {c.name for c in await store.list_channels()}
        assert channels == {"ch2", "ch3"}

    @pytest.mark.asyncio
    async def test_delete_cleared_channel_removes_it(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_removes_event(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_message_removes_by_id(self) -> None:
        store = MessageStore()
    ...
    @pytest.mark.asyncio
    async def test_delete_message_nonexistent_id(self) -> None:
        store = MessageStore()
        await store.add("test", "msg")

        result = await store.delete_message("test", "00000000-0000-0000-0000-000000000000")
        assert result is False
        msgs, _ = await store.get("test")
        assert len(msgs) == 1

    @pytest.mark.asyncio
    async def test_delete_message_nonexistent_channel(self) -> None:
        store = MessageStore()
        # Use a valid UUID; channel does not exist
        result = await store.delete_message(
            "no-such-channel", "00000000-0000-0000-0000-000000000000"
        )
        assert result is False

    @pytest.mark.asyncio
    async def test_delete_message_preserves_order(self) -> None:
        store = MessageStore()
        msgs = []
        for i in range(5):
            msgs.append(await store.add("test", f"msg-{i}"))

        await store.delete_message("test", msgs[2].id)

        remaining, _ = await store.get("test")
        assert [m.content for m in remaining] == ["msg-0", "msg-1", "msg-3", "msg-4"]

    @pytest.mark.asyncio
    async def test_delete_message_from_full_channel(self) -> None:
        ...
        result = await store.delete_message("test", target_id)
        assert result is True
        remaining, _ = await store.get("test")
    ...
    @pytest.mark.asyncio
    async def test_delete_message_sender_match_succeeds(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_message_sender_mismatch_denied(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_message_no_sender_deletes_any(self) -> None:
        """delete_message without sender constraint removes regardless of sender."""
        store = MessageStore()
        msg = await store.add("test", "content", "alice")

        result = await store.delete_message("test", msg.id)
        assert result is True
        msgs, _ = await store.get("test")
        assert msgs == []

    @pytest.mark.asyncio
    async def test_max_messages_eviction(self) -> None:
        store = MessageStore(max_per_channel=5)
        for i in range(10):
            await store.add("test", f"msg-{i}")
    ...
    async def test_message_to_dict(self) -> None:
        store = MessageStore()
        msg = await store.add("test", "hello", "bob")
        d = msg.to_dict()
    ...
    @pytest.mark.asyncio
    async def test_default_sender(self) -> None:
        store = MessageStore()
        msg = await store.add("test", "hello")
        assert msg.sender == "anonymous"

    @pytest.mark.asyncio
    async def test_max_channels_enforced(self) -> None:
        ...
        with pytest.raises(ValueError, match="Channel limit reached"):
            await store.add("ch4", "msg")

    @pytest.mark.asyncio
    async def test_max_message_size_enforced(self) -> None:
        store = MessageStore(max_message_size=10)
        await store.add("test", "short")  # OK

        with pytest.raises(ValueError, match="Message too large"):
            await store.add("test", "x" * 11)

    @pytest.mark.asyncio
    async def test_sort_order_desc_default(self) -> None:
        """Default sort_order='desc' returns the newest N messages."""
        store = MessageStore()
        for i in range(5):
            await store.add("test", f"msg-{i}")

        messages, _ = await store.get("test", limit=3)
        assert len(messages) == 3
    ...
    @pytest.mark.asyncio
    async def test_sort_order_desc_explicit(self) -> None:
        """Explicit sort_order='desc' returns the newest N messages."""
        store = MessageStore()
        for i in range(5):
            await store.add("test", f"msg-{i}")

        messages, _ = await store.get("test", limit=3, sort_order="desc")
        assert len(messages) == 3
        assert messages[0].content == "msg-2"
        assert messages[2].content == "msg-4"

    @pytest.mark.asyncio
    async def test_sort_order_asc_returns_oldest_first(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_sort_order_asc_with_since(self) -> None:
        """sort_order='asc' combined with since filters correctly."""
        store = MessageStore()
        msg0 = await store.add("test", "msg-0")
        await store.add("test", "msg-1")
        await store.add("test", "msg-2")
        await store.add("test", "msg-3")

        messages, _ = await store.get("test", since=msg0.timestamp, limit=2, sort_order="asc")
        assert len(messages) == 2
        assert messages[0].content == "msg-1"
        assert messages[1].content == "msg-2"

    @pytest.mark.asyncio
    async def test_sort_order_desc_with_since(self) -> None:
        ...
        messages, _ = await store.get("test", since=msg0.timestamp, limit=2, sort_order="desc")
        assert len(messages) == 2
        assert messages[0].content == "msg-2"
        assert messages[1].content == "msg-3"

    @pytest.mark.asyncio
    async def test_sort_order_invalid_raises(self) -> None:
...
class TestCursorPagination:
    ...
    async def _add_messages(self, store: MessageStore, channel: str, count: int) -> list[str]:
        """Add ``count`` messages and return their IDs in order."""
        ids = []
        for i in range(count):
            msg = await store.add(channel, f"msg-{i}")
            ids.append(msg.id)
        return ids
    ...
    @pytest.mark.asyncio
    async def test_after_returns_messages_after_cursor(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 5)
    ...
    @pytest.mark.asyncio
    async def test_after_last_message_returns_empty(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 3)

        messages, has_more = await store.get("test", after=ids[-1])
        assert messages == []
        assert has_more is False

    @pytest.mark.asyncio
    async def test_after_first_message_returns_rest(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 4)

        messages, _ = await store.get("test", after=ids[0])
        assert len(messages) == 3
        assert messages[0].content == "msg-1"

    @pytest.mark.asyncio
    async def test_after_invalid_id_raises(self) -> None:
        store = MessageStore()
        await self._add_messages(store, "test", 3)

        with pytest.raises(ValueError, match="Cursor ID not found"):
            await store.get("test", after="nonexistent-id")

    # ----------------------------------------------------------------- before

    @pytest.mark.asyncio
    async def test_before_returns_messages_before_cursor(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 5)
    ...
    @pytest.mark.asyncio
    async def test_before_first_message_returns_empty(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 3)

        messages, has_more = await store.get("test", before=ids[0])
        assert messages == []
        assert has_more is False

    @pytest.mark.asyncio
    async def test_before_last_message_returns_rest(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 4)

        messages, _ = await store.get("test", before=ids[-1])
        assert len(messages) == 3
        assert messages[-1].content == "msg-2"

    @pytest.mark.asyncio
    async def test_before_invalid_id_raises(self) -> None:
        store = MessageStore()
        await self._add_messages(store, "test", 3)

        with pytest.raises(ValueError, match="Cursor ID not found"):
            await store.get("test", before="nonexistent-id")

    # --------------------------------------------------------------- has_more

    @pytest.mark.asyncio
    async def test_has_more_true_when_results_exceed_limit(self) -> None:
        store = MessageStore()
        await self._add_messages(store, "test", 10)

        messages, has_more = await store.get("test", limit=5)
        assert len(messages) == 5
        assert has_more is True

    @pytest.mark.asyncio
    async def test_has_more_false_when_results_fit_in_limit(self) -> None:
        store = MessageStore()
        await self._add_messages(store, "test", 3)

        messages, has_more = await store.get("test", limit=10)
        assert len(messages) == 3
        assert has_more is False

    @pytest.mark.asyncio
    async def test_has_more_false_when_exactly_limit(self) -> None:
        store = MessageStore()
        await self._add_messages(store, "test", 5)
    ...
    @pytest.mark.asyncio
    async def test_has_more_with_after_cursor(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 10)

        # after first message → 9 remaining; with limit=5 → has_more=True
        messages, has_more = await store.get("test", after=ids[0], limit=5)
        assert len(messages) == 5
        assert has_more is True

    @pytest.mark.asyncio
    async def test_has_more_with_before_cursor(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 10)
        ...
        messages, has_more = await store.get("test", before=ids[-1], limit=5)
        assert len(messages) == 5
        assert has_more is True
    ...
    @pytest.mark.asyncio
    async def test_before_returns_most_recent_within_limit(self) -> None:
        ...
        messages, has_more = await store.get("test", before=ids[9], limit=3)
        assert len(messages) == 3
        assert has_more is True
    ...
    @pytest.mark.asyncio
    async def test_after_combined_with_since(self) -> None:
        store = MessageStore()
        msg0 = await store.add("test", "msg-0")
        await store.add("test", "msg-1")
        msg2 = await store.add("test", "msg-2")
        await store.add("test", "msg-3")

        # since filters to msg-1, msg-2, msg-3; after=msg2.id → msg-3 only
        messages, _ = await store.get("test", since=msg0.timestamp, after=msg2.id)
        assert len(messages) == 1
        assert messages[0].content == "msg-3"

    @pytest.mark.asyncio
    async def test_after_combined_with_limit_has_more(self) -> None:
        store = MessageStore()
        ids = await self._add_messages(store, "test", 20)

        # after the 5th → 15 remaining; limit=5 → has_more=True
        messages, has_more = await store.get("test", after=ids[4], limit=5)
        assert len(messages) == 5
        assert has_more is True

    @pytest.mark.asyncio
    async def test_sequential_pagination_forward(self) -> None:
        ...
        """
        store = MessageStore()
        all_ids = await self._add_messages(store, "test", 10)
        ...
        while True:
            messages, has_more = await store.get("test", after=cursor, limit=3)
            if not messages:
                break
    ...
    @pytest.mark.asyncio
    async def test_sequential_pagination_all_ids_match(self) -> None:
        """IDs returned during `after` pagination match insertion order."""
        store = MessageStore()
        all_ids = await self._add_messages(store, "test", 9)
...
class TestStoreIntegration:
    ...
    @pytest.mark.asyncio
    async def test_send_read_roundtrip(self) -> None:
        store = MessageStore()
        msg = await store.add("e2e", "test message", "sender-a")

        messages, _ = await store.get("e2e")
        assert len(messages) == 1
    ...
    async def test_list_and_clear(self) -> None:
        store = MessageStore()
        await store.add("ch1", "msg1")
        await store.add("ch2", "msg2")

        channels = await store.list_channels()
        assert len(channels) == 2

        await store.clear("ch1")
        msgs, _ = await store.get("ch1")
        assert msgs == []
        msgs2, _ = await store.get("ch2")
        assert len(msgs2) == 1
    ...
    async def test_limit_respected(self) -> None:
        ...
        for i in range(MAX_READ_LIMIT + 50):
            await store.add("big", f"msg-{i}")

        messages, _ = await store.get("big", limit=MAX_READ_LIMIT + 100)
        assert len(messages) == MAX_READ_LIMIT

    @pytest.mark.asyncio
    async def test_delete_channel_integration(self) -> None:
        """delete_channel fully removes a channel; clear_channel keeps the entry."""
        store = MessageStore()
        await store.add("keep", "msg1")
        await store.add("remove", "msg2")

        channels = await store.list_channels()
        assert len(channels) == 2

        # clear_channel keeps the channel in the listing
        await store.clear("keep")
        channels = await store.list_channels()
        assert len(channels) == 2
        by_name = {c.name: c for c in channels}
