...
describe('Auth', () => {
  ...
  describe('User creation', () => {
    it('should create new user successfully', async () => {
      ...
      const result = await Auth.createUser('testuser', 'test@example.com', 'password123');
      
      expect(result).toBeDefined();
      expect(TodoDB.TodoDB.getUserByUsername).toHaveBeenCalledWith('testuser');
      expect(TodoDB.TodoDB.getUserByEmail).toHaveBeenCalledWith('test@example.com');
      expect(TodoDB.TodoDB.createUser).toHaveBeenCalledWith('testuser', 'test@example.com', expect.any(String));
    });

    it('should throw error if username already exists', async () => {
      TodoDB.TodoDB.getUserByUsername.mockResolvedValue({ id: 1, username: 'existinguser' });
      TodoDB.TodoDB.getUserByEmail.mockResolvedValue(undefined);

      await expect(Auth.createUser('existinguser', 'test@example.com', 'password123'))
        .rejects.toThrow('User already exists');
    });

    it('should throw error if email already exists', async () => {
      TodoDB.TodoDB.getUserByUsername.mockResolvedValue(undefined);
      TodoDB.TodoDB.getUserByEmail.mockResolvedValue({ id: 1, email: 'existing@example.com' });

      await expect(Auth.createUser('testuser', 'existing@example.com', 'password123'))
        .rejects.toThrow('User already exists');
    });
  });
  ...
  describe('User authentication', () => {
    ...
    it('should throw error for non-existent user', async () => {
      ...
      await expect(Auth.authenticateUser('nonexistent', 'password123'))
    ...
    it('should throw error for incorrect password', async () => {
      ...
      await expect(Auth.authenticateUser('testuser', wrongPassword))
        .rejects.toThrow('Invalid credentials');
    });
  });
  ...
  describe('Session management', () => {
    it('should create session', async () => {
      ...
      expect(TodoDB.TodoDB.createSession).toHaveBeenCalledWith(expect.any(String), 1);
    });
