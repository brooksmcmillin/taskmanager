...
async def token_endpoint(
    ...
    if not client.is_public and client.client_secret_hash:
...
async def _handle_authorization_code(
    db, client: OAuthClient, code: str | None, redirect_uri: str | None, code_verifier: str | None
) -> dict:
...
async def _handle_refresh_token(db, client: OAuthClient, refresh_token: str | None) -> dict:
    ...
    result = await db.execute(
        select(AccessToken).where(
            ...
            AccessToken.refresh_token_expires_at > datetime.now(timezone.utc),
        )
    )
...
async def _handle_client_credentials(db, client: OAuthClient, scope: str | None) -> dict:
...
async def _handle_device_code(db, client: OAuthClient, device_code: str | None) -> dict:
    ...
    # Check polling rate
    if dc.last_poll:
        elapsed = (datetime.now(timezone.utc) - dc.last_poll).total_seconds()
        if elapsed < dc.interval:
            raise errors.oauth_slow_down()

    dc.last_poll = datetime.now(timezone.utc)
